...
function updateDateTime() {
    const birthDate = new Date(2011, 01, 04);
    const now = new Date();
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    btnSurprise.addEventListener("click", () => {
      // Mostrar el modal si aún no se muestra
      modal.classList.add("active");

      // Hacer scroll hasta el modal
      modal.scrollIntoView({ behavior: "smooth" });

      // Aquí puedes llamar también a la música y animaciones si deseas
    });
  });
...
// Reproducción automática al interactuar
...
// -------- Hacer botón draggable --------
let isDragging = false, offsetX, offsetY;

musicBtn.addEventListener("mousedown", (e) => {
  isDragging = true;
  offsetX = e.clientX - musicBtn.offsetLeft;
  offsetY = e.clientY - musicBtn.offsetTop;
  musicBtn.style.cursor = "grabbing";
});

document.addEventListener("mousemove", (e) => {
  if (isDragging) {
    e.preventDefault();
    musicBtn.style.left = `${e.clientX - offsetX}px`;
    musicBtn.style.top = `${e.clientY - offsetY}px`;
    musicBtn.style.bottom = "auto"; // Evitar interferencia con estilos previos
  }
});

document.addEventListener("mouseup", () => {
  isDragging = false;
  musicBtn.style.cursor = "grab";
});
...
musicBtn.addEventListener("touchstart", (e) => {
  ...
  offsetX = touch.clientX - musicBtn.offsetLeft;
  offsetY = touch.clientY - musicBtn.offsetTop;
});

document.addEventListener("touchmove", (e) => {
  if (isDragging) {
    const touch = e.touches[0];
    musicBtn.style.left = `${touch.clientX - offsetX}px`;
    musicBtn.style.top = `${touch.clientY - offsetY}px`;
...
window.addEventListener("load", () => {
  ...
  }, 1000); // Espera 1 segundo antes de intentar reproducir
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    surpriseBtn.addEventListener("click", function () {
      audio.play().catch((e) => {
        console.log("Autoplay bloqueado, intenta con interacción del usuario:", e);
      });
    });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function launchFallingItems() {
      const emojis = ['🎈', '🎁', '🎉', '🎊'];
      for (let i = 0; i < 30; i++) {
        const emoji = document.createElement("div");
        emoji.classList.add("falling-element");
